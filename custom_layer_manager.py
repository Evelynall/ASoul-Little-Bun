...
class CustomLayerManager:
    ...
    def _get_config_file(self):
        if self.character_name:
            return os.path.join('img', self.character_name, 'custom_layers.json')
        return 'custom_layers_global.json'
...
def load_layer_config(character_name):
    ...
    if not character_name:
        return {}
    config_file = os.path.join('img', character_name, 'config.json')
...
class CustomLayerDialog(QDialog):
    ...
    def _connect_extra_signals(self):
        for spin in [self.bg_width_spin, self.bg_height_spin,
                     ...
                     self.sensitivity_spin, self.mouse_width_spin, self.mouse_height_spin,
    ...
    def _on_mouse_height_changed(self, value):
        ...
        if self.mouse_ratio_locked and value > 0:
            ...
            self.mouse_width_slider.blockSignals(False); self.mouse_width_spin.blockSignals(False)
    ...
    def reset_image_settings_to_default(self):
        ...
        keys = [
            ...
            'max_mouse_offset', 'mouse_sensitivity', 'sync_scale_enabled',
    ...
    def load_layer_properties(self):
        ...
        if self._is_default_layer(layer):
            ...
            if filename and self.character_name:
                backup_path = os.path.join('img', self.character_name, filename + '.bak')
    ...
    def select_image(self):
        ...
        if self._is_default_layer(self.current_layer):
            ...
            if not filename or not self.character_name:
                return
            target_path = os.path.join('img', self.character_name, filename)
